...
public class Code5 {
    ...
    private String sql = "SELECT a.book_id, a.title, a.author, a.genre, a.pages, a.rating_spread, a.polarization_score FROM( SELECT a.book_id, b.title, b.author, b.genre, b.pages, ( a.highest_rating - a.lowest_rating) AS rating_spread, a.polarization_score FROM ( SELECT book_id, MAX( session_rating ) AS highest_rating, MIN( session_rating ) AS lowest_rating, ROUND( SUM( IF ( session_rating >= 4 OR session_rating <= 2, 1, 0 ) ) / COUNT(*), 2 ) AS polarization_score, count(*) AS total FROM reading_sessions GROUP BY book_id ) a LEFT JOIN books b ON a.book_id = b.book_id WHERE a.highest_rating >= 4 AND a.lowest_rating <= 2 AND a.polarization_score >= 0.6 AND a.total >= 5 ) a ORDER BY a.polarization_score DESC, a.title DESC";
