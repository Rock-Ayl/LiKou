...
public class Code16 {
    ...
    public int earliestFinishTime(int[] landStartTime, int[] landDuration, int[] waterStartTime, int[] waterDuration) {
        ...
        //陆地最快结束时间
        int landQuickEndTime = Integer.MAX_VALUE;
        //循环
        for (int i = 0; i < landStartTime.length; i++) {
            //计算每个,刷新最快
            landQuickEndTime = Math.min(landQuickEndTime, landStartTime[i] + landDuration[i]);
        }
        //循环
        for (int i = 0; i < waterStartTime.length; i++) {
            //水开始时间
            int startTime = Math.max(waterStartTime[i], landQuickEndTime);
            //刷新结果
            result = Math.min(result, startTime + waterDuration[i]);
        }
        ...
        //水最快最快结束时间
        int waterQuickEndTime = Integer.MAX_VALUE;
        //循环
        for (int i = 0; i < waterStartTime.length; i++) {
            //计算每个,刷新最快
            waterQuickEndTime = Math.min(waterQuickEndTime, waterStartTime[i] + waterDuration[i]);
        }
        //循环
        for (int i = 0; i < landStartTime.length; i++) {
            //陆地开始时间
            int startTime = Math.max(landStartTime[i], waterQuickEndTime);
            //刷新结果
            result = Math.min(result, startTime + landDuration[i]);
        }
    ...
    public static void main(String[] args) {
        System.out.println(new Code16().earliestFinishTime(new int[]{2, 8}, new int[]{4, 1}, new int[]{6}, new int[]{3}));
    }
